#Script adapted from: https://github.com/dvingerh/PyInstaStories
		...
		if hq_videos:
			...
			for index, video in enumerate(list_video_v):
				...
				if not os.path.exists(save_path_final):
					File = Query()
					res = db.search(File.filename == final_filename)
					if res != [] and res[0]['user'] == user_to_check:
						print("[I] File already downloaded before")
						continue
					print("[I] ({:d}/{:d}) Downloading video: {:s}".format(index+1, len(list_video_v), final_filename))
					db.insert({'filename': final_filename, 'timestamp': formatDate(datetime.datetime.now()), 'user': user_to_check})
					try:
